...
country_table <- function(df, status) {
  ...
  bens <- df %>%
    filter(Typeofbeneficiary != "Total" & Typeofbeneficiary != "Unknown" &
             Levelofeducation == "Total" &
             Gender != "Total") %>%
    group_by(Country, Typeofbeneficiary) %>%
    summarise(Total = sum(Number, na.rm = TRUE))%>%
  ...
  edu_level <- df %>%
    filter(Typeofbeneficiary == "Total" &
             ...
             Gender != "Total") %>%
    group_by(Country, Levelofeducation) %>%
    summarise(Total = sum(Number, na.rm = TRUE)) %>%
  ...
  edu_type <- df %>%
    filter(Typeofbeneficiary == "Total" &
             Levelofeducation == "Total" &
             Gender != "Total") %>%
    group_by(Country, Typeofeducation) %>%
    summarise(Total = sum(Number, na.rm = TRUE)) %>%
  ...
  df %<>%
    left_join(total, by = "Country") %>%
    left_join(gender, by = "Country") %>%
    left_join(bens, by = "Country") %>%
    left_join(edu_level, by = "Country") %>%
    left_join(edu_type, by = "Country")
