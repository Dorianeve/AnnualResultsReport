...
today <- today()
folder_path <- paste0("data/output/aoe/", Sys.Date(), "/analysis/")
dir.create(folder_path, recursive = TRUE, showWarnings = FALSE)
wb <- createWorkbook()

df <- read.csv(paste0("data/input/", aoe), encoding = "UTF-8")
...
file_path <- paste0(folder_path, " AoE Analysis - Quantitative.xlsx")
...
df_cr <- read.csv("data/cleaned/CR Combiner - For Analysis.csv")

cr_nfe <- df_cr %>% 
  filter(Exercise == "ARR24 Annual" & Typeofreporting == "Joint" & Status == "Reached" & Typeofeducation == "Non-formal education") %>% 
  group_by(LeadGRN) %>% 
...
myrp <- myrp %>% rename(MYRP = Number)
myrp_fer <- myrp_fer %>% rename(MYRP_FER = Number)
fer <- fer %>% rename(FER = Number)


# Extract names of first columns
key1 <- names(myrp_fer)[1]
key2 <- names(myrp)[1]
key3 <- names(fer)[1]

# Perform full joins by first column in each data frame
data <- myrp_fer %>%
  full_join(myrp, by = setNames(key2, key1)) %>%
  full_join(fer,  by = setNames(key3, key1)) %>%
  rename(Indicator14 = `Indicator14_MYRP|FER`)
