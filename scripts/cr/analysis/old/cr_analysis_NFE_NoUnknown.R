...
# CR ANALYSIS ANNUAL ----
df <- read.csv("data/cleaned/CR Combiner - For Analysis_NoUnknown.csv", encoding = "UTF-8")
...
df_filtered <- df %>%
  filter(
    get(paste0("Activein", report_year)) == "Yes" &
      exercice == annual & 
      Status == "Reached" &
      Typeofeducation == "Non-formal education" &
...
# Save the workbook
saveWorkbook(wb, paste0(folder_path, "Children Reached annual NFE_NoUnknown.xlsx"), overwrite = TRUE)
...
# Save the workbook
saveWorkbook(wb, paste0(folder_path, "Children Reached cumulative strategic NFE_NoUnknown.xlsx"), overwrite = TRUE)
...
# Save the workbook
saveWorkbook(wb, paste0(folder_path, "Children Reached cumulative inception NFE_NoUnknown.xlsx"), overwrite = TRUE)
