...
today <- today()
folder_path <- paste0("data/output/cr/", Sys.Date(), "/Targeted NFE/")
...
df %<>%
  filter(Status == "Targeted" &  Typeofeducation == "Non-formal education")
...
gender_check <- df_filtered %>%
  group_by(LeadGRN, Typeofreporting, Gender) %>%
...
bens_check <- df_filtered %>%
  group_by(LeadGRN, Typeofreporting, Typeofbeneficiary) %>%
  summarise(Total = sum(Number, na.rm = TRUE)) %>%
...
edu_check <- df_filtered %>%
  group_by(LeadGRN, Typeofreporting, Levelofeducation) %>%
  summarise(Total = sum(Number, na.rm = TRUE)) %>%
  pivot_wider(names_from = c(Typeofreporting, Levelofeducation), values_from = Total) %>%
  mutate(across(everything(), ~ replace_na(., 0))) %>%
  mutate(JointComputed = Joint_Primary + Joint_Secondary + Joint_Total + 
           `Joint_Unknown level of education` + `Joint_Pre-primary`, 
         IndividualComputed = Individual_Primary + Individual_Secondary + Individual_Total + 
...
rm(bens_check, checks_list, edu_check, gender_check, bens_check,
